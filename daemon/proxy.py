...
def resolve_routing_policy(hostname, routes):
    ...
    if isinstance(proxy_map, list):
        ...
        # List proxy_map has 1 
        elif len(proxy_map) == 1:
            proxy_host, proxy_port = proxy_map[0].split(":", 2)

        # apply the policy handling 
        #   proxy_map
        #   policy
        else: # len(proxy_map) > 1
            # Out-of-handle mapped host
            # biet la round-robin nhung tam thoi random vay di
            index = random.randint(0, len(proxy_map))
            print(f"[Proxy] Policy '{policy}' triggered. Selecting first server.")
...
def run_proxy(ip, port, routes):
    ...
    try:
        ...
        while True:
            ...
            #
            #  xTODO: implement the step of the client incomping connection
            #        using multi-thread programming with the
            #        provided handle_client routine
            #
            print("[Proxy] Accepted connection from {}:{}".format(addr[0], addr[1]))

            ## code them thread handle
